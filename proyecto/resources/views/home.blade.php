...
$(document).ready(function() {
    ...
    $('#dataTab').on('click', 'tbody td.row-char', function(e) {
        var url = '{{ route("proyectos.graphic", ":id") }}';
        url = url.replace(':id', this.parentNode.id);
        var id = url.replace(/[^0-9]/ig, '');
        openiframe('Gráfica visual', url);
    });
    ...
    editorTab = $('#dataTab').DataTable({
        ...
        columns: [{
            ...
            {
                data: null,
                defaultContent: '<i class="fa fa-area-chart"/>',
                className: 'row-char dt-center',
                orderable: false
            }
        ],
        lengthMenu: [
